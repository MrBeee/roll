...
Building templates is normally hidden from sight when you use the wizards to define the survey geometry.
However, you may want to make some changes to these templates and then some understanding how templates are constructed is very helpful.

Receiver Seed location
----------------------
Building a template is a multi-step process. The first step is defining the location of the receiver seed.
...
There need to be at least one source seed and one receiver seed in each template to define trace locations.
Some more complex geometries (slant, zigzag) require several source seeds defining multiple shot salvos in a shared receiver spread, before move-up occurs.
...
The number of templates in a survey is two (*for each sail line direction*) times the number of race tracks times the number of towed sources.

.. note::

   For Roll the streamer templates are demanding to display, due to the large number of templates with their large overlap. For this reason Level of Detail (LOD) drawing has been implemented, so the GUI stays responsive when zooming in or out.
...
The U-Turn:
   When completing the run-out, the sources are temporarily powered down. The ship executes a wide, sweeping loop.
...
.. note::
   ...
   In the middle of a race track, the sail line direction reverses. The presence of cross-currents can cause deviation (*feathering*) of the streamers and this will lead to gaps in the subsurface coverage
...
.. note::
   ...
   By using an **odd** number of sail lines in a race track, one can prevent a reversal of the sail line direction between race tracks. This is shown in the above picture, that contains two racetracks, each with 19 sail lines.
