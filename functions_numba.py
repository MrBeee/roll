...
# @jit(nb.types.Tuple((nb.float32[:], nb.float32[:], nb.boolean))(nb.float32[:, :, :, :], nb.boolean), nopython=True)                    # numba needs array specs to work
...
def numbaNdft2D(kMin: float, kMax: float, dK: float, offsetX: np.ndarray, offsetY: np.ndarray):
    ...
    for x in prange(nX):                                                        # execute parallel loops
        for y in range(nY):
            # Vectorized inner sum over all points p
            response = np.sum(np.exp(2j * np.pi * (kX[x] * offsetX + kY[y] * offsetY)))
...
# @jit('void(numType1[:], i8, f8, f8, i8, f8, f8, f4[3])', nopython=True)
...
def numbaBinBatchParallel(
    ...
    numShots = len(srcBatch)
    nx_max, ny_max = binOutput.shape
    ...
    for i in range(numShots):
        ...
        # Optimization: Only iterate over relations for this shot
        for relIdx in range(minR, maxR):
            rMin = relRecMinI[relIdx]
            ...
            for rIdx in range(rStart, rEnd):
                if recPointI[rIdx] >= rMin and recPointI[rIdx] <= rMax:
