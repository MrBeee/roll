@echo off
rem preferred usage: ".\run_flake8_qgis_strict.bat --report flake8-strict-report.txt"
...
if not defined reportFile if defined FLAKE8_REPORT_FILE set "reportFile=%FLAKE8_REPORT_FILE%"
...
call :runFlake8 "%osgeoRoot%\bin\python.exe"
set "lintExitCode=%ERRORLEVEL%"
goto :reportResult
...
if defined reportFile (
    echo Writing strict flake8 output to "%reportFile%"
    call "%pythonCmd%" -m flake8 . --select=%strictSelect% > "%reportFile%" 2>&1
) else (
    call "%pythonCmd%" -m flake8 . --select=%strictSelect%
)
exit /b %ERRORLEVEL%
...
if "%lintExitCode%"=="0" (
    echo.
    echo STRICT FLAKE8 PASSED
) else (
    echo.
    echo STRICT FLAKE8 FINISHED WITH ISSUES ^(exit code %lintExitCode%^)
    echo Checked codes: %strictSelect%
)
