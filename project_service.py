    ...
    def _loadAnalysisArraySidecars(self, fileName, survey, result):
        ...
        if cols == 13:
            self._appendMessage(
                result,
                'error',
                f'Loaded : . . . Analysis &nbsp;: legacy 13-column layout detected for {anaPath}. The format was extended with src-z, rec-z and cmp-z columns; please re-run the binning to refresh the trace table.',  # noqa: E501 # pylint: disable=C0301
            )
            return
        if cols != 16:
            self._appendMessage(
        ...
        if memmapResult.memmap.mode == 'c':
            self._appendMessage(
                result,
                'warning',
                f'Loaded : . . . Analysis &nbsp;: opened copy-on-write because writable access was denied for {fileName + ".ana.npy"}. Trace records are available, but in-memory edits will not be saved. {memmapResult.errorText}',  # noqa: E501 # pylint: disable=C0301
            )
        ...
        if result.maximumFold > fold:
            self._appendMessage(result, 'info', f'Loaded : . . . Analysis &nbsp;: observed fold in binning file: {result.maximumFold:,}. This is larger than allowed in the trace table ({fold:,}), expect missing traces in spider plot !')  # noqa: E501 # pylint: disable=C0301
