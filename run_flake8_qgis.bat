@echo off
rem preferred usage: ".\run_flake8_qgis.bat --report flake8-report.txt"
...
if defined reportFile (
    echo Writing flake8 output to "%reportFile%"
    call "%pythonCmd%" -m flake8 . > "%reportFile%" 2>&1
) else (
    call "%pythonCmd%" -m flake8 .
)
exit /b %ERRORLEVEL%
...
if "%lintExitCode%"=="0" (
    ...
    echo.
    echo FLAKE8 FINISHED WITH ISSUES ^(exit code %lintExitCode%^)
)
